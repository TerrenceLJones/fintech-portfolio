...
async function bootstrap() {
  if (import.meta.env.DEV) {
    ...
    const { worker, simulateLoginFailure } =
      await import('@fintech-portfolio/mock-backend/browser');
    await worker.start({ onUnhandledRequest: 'bypass' });
    // e2e-only control surface (apps/clearline-web/e2e/login.spec.ts AC-05) — Playwright drives
    // this via page.evaluate() since it can't intercept MSW's Service Worker-mocked requests.
    window.__e2eMockBackend = { simulateLoginFailure };
  }
